...
def main() -> None:
    ...
    with st.sidebar:
        ...
        # Restart the assistant if the llm_id changes.
        ...
        # Get calculator_enabled fro session state if set.
        calculator_enabled = st.session_state['calculator_enabled']
        # Checkbox for enabling claculator.
        calculator = st.checkbox(
            'Calculator',
            value=calculator_enabled,
            help='Enable calcualtor.'
        )
        ...
        # Checkbox for enabling file tools
        file_tools = st.checkbox(
            'File Tools',
            value=file_tools_enabled,
            help='Enable file tools.',
        )
        ...
        shell_tools = st.checkbox(
            'Shell Tools',
            value=shell_tools_enabled,
            help='Enable shell tools.',
        )
        ...
        # Checkbox for enabling data analyst.
        data_analyst = st.checkbox(
            'Data Analyst',
            value=data_analyst_enabled,
            help='Enable the Data Analyst assistant for the data related queries.',
        )
        ...
        # Checkbox for enabling python assistant.
        python_assistant = st.checkbox(
            'Python Assistant',
        ...
        # Checkbox for enabling research assistant.
        research_assistant = st.checkbox(
            'Research Assistant',
        ...
        # Checkbox for enabling investment assistant.
        investment_assistant = st.checkbox(
        ...
        if llm_os.knowledge_base:
            ...
            if uploaded_file is not None:
                ...
                if f'{auto_rag_name}_uploaded' not in st.session_state:
                    reader = PDFReader()
                    auto_rag_documents: list[Document] = reader.read(
                        uploaded_file
                    )
    ...
    with st.sidebar:
        if llm_os.storage:
            ...
            new_llm_os_run_id = st.selectbox(
                'Run ID',
                options=llm_os_run_ids,
            )
